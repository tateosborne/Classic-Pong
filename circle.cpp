...
bool Circle::isOverlappingLeftPaddle(const Rect &r) const {

    return (r.getRightX() > getLeftX() && (r.getTopY() < getTopY() && r.getBottomY() > getBottomY()));
}

bool Circle::isOverlappingRightPaddle(const Rect &r) const {

    return (r.getLeftX() < getRightX() && (r.getTopY() < getTopY() && r.getBottomY() > getBottomY()));
}
